export default class SortableTable {
    ...
    getSortables() {
        return this.element.querySelectorAll('[data-sortable]');
    }

    setOnclick(  ) {
        const sortables = this.getSortables();
        for( let item of sortables ) {
            if( item.dataset.id === 'price' || 'title' ) item.dataset.order = 'asc';
            if( item.dataset.sortable !== 'false') {
                item.addEventListener( 'pointerdown', () => {
                    item.dataset.order = item.dataset.order === 'asc' ? 'desc' : 'asc';
